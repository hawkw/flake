let
  ...
  loadHosts = dir: inputs:
    let
      ...
      hosts' = let contents = readDir dir;
    ...
    in concatMap (n:
      ...
      in if hasDefault then [ (loadConf dir n) ] else [ ]) hosts';
in {
  ...
  genHomeHosts = { inputs, user, directory ? "${inputs.self}/hosts"
    ...
    , baseModules ? [ ], overlays ? [ ], config ? { allowUnfree = true; }, }:
    let
      homeHosts = concatMap (h:
